...
def routes_with_scales_support(mins_scales, _max_scales, source, destination, code_type,page):
    ...
    return f"MATCH p=(n:Airport {{{from_code_condition}}}) -[:HAS_ROUTE_TO*{scales_multiplier}]->(m:Airport{{{destination_code_condition}}}) WITH *, relationships(p) as r WHERE SIZE(apoc.coll.toSet(NODES(p))) > LENGTH(p) RETURN r,nodes(p) skip {(page-1)*page_size} limit {page_size}"


def airports_reachable(source, scales, code_type,page):
    scales += 1
    from_code_condition = f"icao: \"{source}\"" if code_type == "ICAO" else f"iata: \"{source}\""
    return f"MATCH p=(n:Airport {{{from_code_condition}}}) -[:HAS_ROUTE_TO*{scales}]->(m:Airport) WITH *,relationships(p) as r WHERE SIZE(apoc.coll.toSet(NODES(p))) > LENGTH(p) RETURN p,r skip {(page-1)*page_size} limit {page_size}"
...
def all_routes_between_two_airports_avoiding_airline(source, destination, code_type, airline_code, airline_code_type,page):
    from_code_condition = f"icao: \"{source}\"" if code_type == "ICAO" else f"iata: \"{source}\" "
    destination_code_condition = f"icao: \"{destination}\"" if code_type == "ICAO" else f"iata: \"{destination}\" "
    ...
    return (f"MATCH p=(n:Airport {{{from_code_condition}}})-[r:HAS_ROUTE_TO*1..{max_scales + 1}]->(m:Airport {{{destination_code_condition}}}) "
            ...
            f"return p,r skip {(page-1)*page_size} limit {page_size}")


def all_routes_between_two_airports_avoiding_airport(source, destination, avoid, code_type,page):
    from_code_condition = f"icao: \"{source}\"" if code_type == "ICAO" else f"iata: \"{source}\" "
    destination_code_condition = f"icao: \"{destination}\"" if code_type == "ICAO" else f"iata: \"{destination}\" "
